...
class SQLiteQuestionRepository(IQuestionRepository):
    ...
    def list_all(
        ...
        with get_connection(self.db_path) as conn:
            ...
            if exam_track:
                query += " AND exam_track = ?"
                params.append(exam_track.value)
    ...
    def _row_to_question(self, row) -> Question:
        ...
        return Question(
            ...
            exam_track=ExamTrack(row["exam_track"]) if row["exam_track"] else None,
            points=row["points"] or 1,
            image_path=row["image_path"],
            is_validated=bool(row["is_validated"]) if row["is_validated"] else False,
            validation_notes=row["validation_notes"] if "validation_notes" in row.keys() else None,
