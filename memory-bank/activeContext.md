...
**2026-07-07 補充：已把 `codex/openclaw-telegram-admin` 線上的 OpenClaw Telegram 管理、前端代理與 answer 值物件工作整併回 `master`，並收斂本地分支（fast-forward）。這一段落把 OpenClaw 從「純後端 agent」延伸到可操作介面：新增 `scripts/openclaw_front_proxy.py`（瀏覽器可見的 Gateway 前端代理）、Streamlit 右側嵌入式 OpenClaw console (`src/presentation/streamlit/chat_panel.py`)、`OPENCLAW_GATEWAY_PUBLIC_URL` 設定，以及 Telegram 管理面（`telegram_admin_service.py`、`run_telegram_admin_bot.py`、`run_telegram_status_report.py`）與 backlog worker（`openclaw_backlog_worker.py`、`openclaw_session_keys.py`、`run_openclaw_heartbeat_worker.py`）對應的 systemd service/timer。domain 層新增 `value_objects/answer.py`，統一答案字母正規化/格式化與單選/多選題型判定；`exam_tool_application_service`、MCP exam server/handlers 與 crush streaming 也一併重構。文件面補上 `AGENTS.md`/`TOOLS.md`/`USER.md`、asset-aware document agent 與 pdf-asset-extractor skill，並更新 CHANGELOG/README。`.gitignore` 已收斂 AI 工具本地執行狀態/備份（`.codex/`、`.openclaw/`、`.cline/` 等）。本地分支已收斂為單一 `master`。**

**2026-05-26 補充：OpenClaw repo-local runtime 已升級並穩定在 `2026.5.22`，網站不再共用爆掉的 `agent:main:main` session。Provider、Web chat、generation、ETL、worker、scope dispatch 與 Telegram `/ask` 入口已改用穩定 session-key 分流：Web 依 browser session/question、worker 依 `job_id`、scope 依 `scope_request_id`、Telegram 依 `chat_id`。舊 `agent:main:main` 已封存到 OpenClaw state archive。為避免多人新 session 仍因 MCP/skills/tool schema 過大而 context overflow，repo-local OpenClaw config 已啟用 `tools.toolSearch`、`agents.defaults.experimental.localModelLean=true`、`contextInjection=continuation-skip`、bootstrap caps 與題庫/教材管理 skill allowlist；同樣設定已寫入 `scripts/configure_openclaw_gb10.sh` 供重建部署使用。Smoke 結果：direct OpenClaw agent fresh session 回 `OK`，system prompt tools schema 降為單一 `tool_search_code`，Python provider smoke 回 `OK`，`anesthesia-exam-web.service`、`anesthesia-exam-openclaw-worker.timer`、`openclaw-gateway.service` 皆 active，`http://127.0.0.1:8501` 回 `200 OK`。**
...
**2026-05-13 hotfix 補充：asset-aware 已追加發布 `v0.6.31`，專門修復 VSIX `npm audit` 的 high severity finding。root cause 是 VSIX dev dependency chain `@vscode/vsce -> @secretlint/node -> @secretlint/config-loader -> ajv -> fast-uri` 鎖到 vulnerable `fast-uri@3.1.0`；hotfix 透過 lockfile 更新讓 `fast-uri` 解析到 `3.1.2`。驗證結果：`npm audit --audit-level=high` 為 `0 vulnerabilities`，`npm ls fast-uri` 顯示 `fast-uri@3.1.2`，VSIX `sync-assets:check/test:ci/test:install-smoke`、Python lint/type/tests、package build、wheel install smoke、Docker smoke、release artifact/harness audit 均已通過。Release URL: `https://github.com/u9401066/asset-aware-mcp/releases/tag/v0.6.31`。**
