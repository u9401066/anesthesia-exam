...
> - **建議流程**：先 `asset-aware__inspect_document_manifest` → 選定目標圖 → 一次 fetch 一張
...
| `asset-aware__inspect_document_manifest` | 查看文件結構（圖/表/章節清單） | `doc_id: str` |
| `asset-aware__fetch_document_asset` | 取得特定資產 | `doc_id`, `asset_type`, `asset_id` |
...
| Tool | 用途 | 參數 |
|------|------|------|
| `asset-aware__search_source_location` | 在 blocks.json 中搜尋特定內容 | `doc_id: str`, `query: str`, `block_types: list` |

### 知識圖譜

| Tool | 用途 | 參數 |
|------|------|------|
| `asset-aware__consult_knowledge_graph` | 跨文獻 RAG 查詢 | `query: str`, `mode: str` |
...
2. 呼叫 asset-aware__inspect_document_manifest
...
```python
list_documents()
asset-aware__inspect_document_manifest(doc_id="abc123def")
```
...
```python
# 特定章節
asset-aware__fetch_document_asset(doc_id="abc123def", asset_type="section", asset_id="sec_methods")

# 全文 Markdown
asset-aware__fetch_document_asset(doc_id="abc123def", asset_type="full_text", asset_id="full")
```
...
2. 選擇查詢模式
3. 呼叫 asset-aware__consult_knowledge_graph
```
...
3. asset-aware__inspect_document_manifest → 了解結構
4. fetch section (Introduction) → 快速瀏覽
5. 根據需要 fetch 特定圖表
6. asset-aware__consult_knowledge_graph → 整合分析
```
...
2. asset-aware__inspect_document_manifest → 找 fig_X_X 對應
3. asset-aware__fetch_document_asset(asset_type="figure", asset_id="fig_3_1")
4. 返回圖片 + metadata
```
...
執行流程：
1. 確認兩份文件都已 ingest
2. asset-aware__consult_knowledge_graph(mode="global")
